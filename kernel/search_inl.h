...
namespace impl
{
  ...
  ///
  /// Reapeatedly calls arl(const indi &) for each member of the team.
  ///
  template<class T>
  void arl(const team<T> &base, const environment &env,
           evaluator<team<T>> &eva, problem *prob)
  {
    for (const auto &ind : base)
      arl(env, prob, eva, ind);
  }
...
{
  impl::arl(base, env_, *active_eva_, prob_);
}
