...
TEST_CASE("Sniffer")
{
  ...
  std::istringstream abalone_h(s_abalone_h);
  std::istringstream abalone_nh(s_abalone_nh);
  csv_parser csv1[2] = {csv_parser(abalone_h), csv_parser(abalone_nh)};
  CHECK(csv1[0].dialect().has_header == true);
  CHECK(csv1[1].dialect().has_header == false);

  std::istringstream iris_h(s_iris_h);
  std::istringstream iris_nh(s_iris_nh);
  csv_parser csv2[2] = {csv_parser(iris_h), csv_parser(iris_nh)};
  CHECK(csv2[0].dialect().has_header == true);
  CHECK(csv2[1].dialect().has_header == false);

  std::istringstream car_speed_h(s_car_speed_h);
  std::istringstream car_speed_nh(s_car_speed_nh);
  csv_parser csv3[2] = {csv_parser(car_speed_h), csv_parser(car_speed_nh)};
  CHECK(csv3[0].dialect().has_header == true);
  CHECK(csv3[1].dialect().has_header == false);

  std::istringstream colors_h(s_colors_h);
  std::istringstream colors_nh(s_colors_nh);
  csv_parser csv4[2] = {csv_parser(colors_h), csv_parser(colors_nh)};
  CHECK(csv4[0].dialect().has_header == true);
  CHECK(csv4[1].dialect().has_header == false);

  std::istringstream addresses(s_addresses);
  csv_parser csv5(addresses);
  CHECK(csv5.dialect().has_header == false);

  std::istringstream air_travel(s_air_travel);
  csv_parser csv6(air_travel);
  CHECK(csv6.dialect().has_header == true);
}
